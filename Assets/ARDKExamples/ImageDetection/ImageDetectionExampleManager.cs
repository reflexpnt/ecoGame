...
namespace Niantic.ARDKExamples
{
    ...
    MonoBehaviour
  {
        ...
        static Dictionary<string, Color> _imageColors = new Dictionary<string, Color>
    {
      { "yeti",Color.green},
      { "Carta",Color.blue},
      { "Plastica",Color.yellow},
      { "Umido", new Color(0.4f,0.2f,0f)},
      { "Vetro",new Color(0.4f,0.7f,0.2f)},
      {"Abiti",new Color(1f,0.6f,0.3f)},
      {"Indifferenziata",Color.gray},
      {"Batterie",new Color(0.4f,0.5f,0f)},
      {"Farmaci",Color.red},
       


    };
        static Dictionary<string, string > _imageTag = new Dictionary<string, string>
    {
      {"yeti","100"},
      {"Umido","0"},
      {"Plastica","1"},
    ...
    private void SetPlaneColor(GameObject plane, string imageName)
    {
      ...
      renderer.material.color = planeColor;
            string TagTemp ="";
            _imageTag.TryGetValue(imageName, out TagTemp);
            plane.gameObject.tag = TagTemp;
            //plane.GetComponentInChildren<TextMesh>().text = TagTemp;
    }
